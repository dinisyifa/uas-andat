...
def popularitas_genre(db: Session = Depends(get_db)):
    ...
    query = text("""
        SELECT 
            m.genre,
            COUNT(os.id) AS total_tiket_terjual,
            ROUND(COUNT(os.id) * 100.0 / (SELECT COUNT(*) FROM order_seats), 1) AS persentase_minat
        FROM order_seats os
...
def popularitas_genre_weekly(db: Session = Depends(get_db)):
    ...
    for i, (start, end) in enumerate(minggu_ranges, start=1):
        ...
        # Query: Hitung jumlah tiket per genre dalam rentang tanggal
        query = text("""
            SELECT 
                m.genre,
                COUNT(os.id) AS total_tiket
            FROM order_seats os
            JOIN orders o ON os.order_id = o.id
            JOIN jadwal j ON o.jadwal_id = j.id
            JOIN movies m ON j.movie_id = m.id
        ...
        # Hitung Total Tiket Minggu Ini untuk Persentase
        total_tiket_mingguan = sum(r["total_tiket"] for r in rows)

        data_genre = []
        for r in rows:
            persentase = 0
            if total_tiket_mingguan > 0:
                persentase = round((r["total_tiket"] / total_tiket_mingguan) * 100, 1)
            
            data_genre.append({
                "genre": r["genre"],
                "total_tiket": r["total_tiket"],
                "persentase_minat": persentase
            })

        hasil_minggu.append({
            "minggu_ke": i,
            "range": f"{start_date.isoformat()} s/d {end_date.isoformat()}",
            "total_tiket_terjual": total_tiket_mingguan,
            "data": data_genre
        })

    return {
        "bulan": "Desember 2024",
        "analisis": "Popularitas Genre Mingguan",
        "hasil": hasil_minggu
    }
...
def popularitas_genre_monthly(bulan: str, db: Session = Depends(get_db)):
    ...
    # Query: Hitung jumlah tiket per genre dalam satu bulan
    query = text("""
        SELECT 
            m.genre,
            COUNT(os.id) AS total_tiket
    ...
    # Hitung Persentase
    total_tiket_bulanan = sum(r["total_tiket"] for r in rows)

    data_genre = []
    for r in rows:
        persentase = 0
        if total_tiket_bulanan > 0:
            persentase = round((r["total_tiket"] / total_tiket_bulanan) * 100, 1)
    ...
    return {
        "bulan": bulan,
        "analisis": "Popularitas Genre Bulanan",
        "total_tiket_terjual": total_tiket_bulanan,
        "data": data_genre
    }